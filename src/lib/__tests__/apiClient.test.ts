...
describe('sanitizeErrorMessage', () => {
  ...
  it('strips query parameters mid-sentence', () => {
    expect(sanitizeErrorMessage('route ?source=USDC&dest=EURC not found')).toBe(
      'route not found'
    );
  });
  ...
  it('does NOT redact a hex string shorter than 20 chars', () => {
    expect(sanitizeErrorMessage('ref: 1a2b3c4d5e6f')).toBe('ref: 1a2b3c4d5e6f');
  });
  ...
  it('redacts a 32-character Base58 token', () => {
    expect(sanitizeErrorMessage('token: 3yMApqCuCjXDWPrbjfR5mjCPTHqFG3u')).toBe(
      'token: [redacted]'
    );
  });
  ...
  it('collapses extra spaces left after stripping', () => {
    expect(sanitizeErrorMessage('error  ?a=b  extra')).toBe('error extra');
  });
  ...
  it('redacts a pk_test_ prefixed key', () => {
    expect(sanitizeErrorMessage('Rejected: pk_test_ZxYwVuTsRqPoNmLk1234')).toBe(
      'Rejected: [redacted]'
    );
  });
...
describe('apiFetch', () => {
  ...
  it('strips query params that the server echoes back in its error message', async () => {
    mockResponse(
      ...
      JSON.stringify({
        ...
        message:
          'Bad request: ?source_asset=USDC&dest_asset=EURC&amount=1000000',
      })
    );
    const err = (await apiFetch(
      '/api/v1/quote?source_asset=USDC&dest_asset=EURC&amount=1000000'
  ...
  it('redacts a Stellar wallet address echoed in an error message', async () => {
    ...
    const err = (await apiFetch('/api/v1/quote').catch(
      (e: unknown) => e
    )) as Error;
...
describe('helper functions', () => {
  ...
  it('apiPost calls apiFetch with POST method and JSON body', async () => {
    ...
    await expect(
      apiPost('/api/v1/pairs', { source: 'USDC', destination: 'EURC' })
  ...
  it('apiPatch calls apiFetch with PATCH method and JSON body', async () => {
    ...
    await expect(
      apiPatch('/api/v1/pairs/1', { active: false })
...
describe('sensitive URL parameter redaction', () => {
  it('does not expose source_asset in the toast-visible error message', async () => {
    mockResponse(
      ...
      JSON.stringify({
        error: 'invalid_params',
        message:
          'Invalid request: ?source_asset=USDC&dest_asset=EURC&amount=5000000',
      })
    );
    const err = (await apiFetch(
  ...
  it('does not expose a raw hex API key echoed in a 401 message', async () => {
    ...
    const unregister = registerAuthErrorHandler(jest.fn());
    const err = (await apiFetch('/api/v1/pairs').catch(
      (e: unknown) => e
    )) as Error;
  ...
  it('sanitizes a message containing both a query param and a secret token', async () => {
    ...
    const err = (await apiFetch('/api/v1/quote?dest_asset=EURC').catch(
      (e: unknown) => e
    )) as Error;
    expect(err.message).not.toContain('EURC');
  ...
  it('strips a webhook secret URL parameter from an error message', async () => {
    ...
    const err = (await apiFetch('/api/v1/webhooks').catch(
      (e: unknown) => e
    )) as Error;
  ...
  it('does not redact short human-readable values (requestId-like strings)', async () => {
    ...
    const err = (await apiFetch('/api/v1/pairs/abc123').catch(
      (e: unknown) => e
    )) as Error & {
      requestId?: string;
    };
