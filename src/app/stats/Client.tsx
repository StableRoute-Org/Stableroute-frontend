...
export default function StatsClient() {
  ...
  useEffect(() => {
    const timer = setInterval(api.refetch, POLL_MS);
  ...
  return (
    ...
    >
      ...
      {(api.status === 'idle' || api.status === 'loading') && (
      ...
      {api.status === 'success' && (
        <section aria-labelledby="stats-metrics-heading">
          ...
          <dl className="grid grid-cols-2 gap-4">
            ...
            <StatTile
              label="Status"
              value={api.data.paused ? 'Paused' : 'Live'}
            />
          </dl>
        </section>
      )}
      {api.status === 'success' && api.data.totalPairs === 0 && (
        <EmptyState
          title="No pairs yet"
